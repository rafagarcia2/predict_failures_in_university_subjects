...
def process_args(args):
    ...
    matriculas = pd.read_csv(artifact_path)
    ...
    columns = ['nota', 'n1_std_turma', 'desempenho_exatas', 'primeira_vez_pagando',
       'historico_reprovacao', 'prof_c1_tx_reprovacao', 'professor_tx_reprovao', 'reprovou']
    
    matriculas = matriculas[columns]

    splits["train"], splits["test"] = train_test_split(
        matriculas,
        test_size=args.test_size,
        random_state=args.random_state,
        stratify=matriculas[args.stratify] if args.stratify != 'null' else None
    )
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        for split, df in splits.items():
            ...
            matriculas.to_csv(temp_path,index=False)
